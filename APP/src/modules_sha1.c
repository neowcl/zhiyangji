...
uint8_t sha1_authenticate(uint8_t *buf, uint8_t *key)
{
    uint8_t len = 0;
    uint8_t digest[20] = {0};
    uint8_t key_len = 16;
    uint8_t message[20] = {0};//20*8=160bit
    uint8_t input[16 + 20] = {0};
    //step 1
    //复制随机数，需要反序
    for(int8_t j = sizeof(message) - 1; j >= 0; j--)
    {
        message[len++] = buf[j];
    }
    memcpy(input, key, key_len);//填充key
    memcpy(&input[16], message, sizeof(message));//填充随机数
    sha1(&digest, input, sizeof(input) * 8);//计算sha1
    //step 2
    memcpy(&input[16], digest, sizeof(digest));//替换随机数
    sha1(&digest, input, sizeof(input) * 8);//计算sha1
    //填充用于上传的结果，需要反序
    len = 0;
    for(int8_t j = sizeof(digest) - 1; j >= 0; j--)
    {
        buf[len++] = digest[j];
    }
    return len;
